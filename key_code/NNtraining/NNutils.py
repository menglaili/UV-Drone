...
from models import CorrResNet34, CorrCtrlNet
...
def setup_cornet(savemodel = True):
    model = CorrCtrlNet()
...
def setup_cornet2(savemodel = True):
    model = CorrCtrlNet()
    state_dict = torch.load('./pretrained/pretrained.pth')
    del state_dict["fc_t.weight"], state_dict["fc_t.bias"]
    state_dict["fc_ctrl1.weight"] = nn.init.kaiming_uniform_(model.fc_ctrl1.weight).float()
    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(model.fc_ctrl1.weight)
    bound = 1 / math.sqrt(fan_in)
    state_dict["fc_ctrl1.bias"] = nn.init.uniform_(model.fc_ctrl1.bias, -bound, bound).float()
    ...
    state_dict["fc_conca1.weight"] = nn.init.kaiming_uniform_(model.fc_conca1.weight).float()
    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(model.fc_conca1.weight)
    bound = 1 / math.sqrt(fan_in)
    state_dict["fc_conca1.bias"] = nn.init.uniform_(model.fc_conca1.bias, -bound, bound).float()
    state_dict["bn_conca1.weight"] = model.bn_conca1.weight.data.fill_(1).float()
    state_dict["bn_conca1.bias"] = model.bn_conca1.bias.data.fill_(0).float()
    state_dict["bn_conca1.running_var"] = model.bn_conca1.running_var.data.fill_(1).float()
    state_dict["bn_conca1.running_mean"] = model.bn_conca1.running_mean.data.fill_(0).float()

    state_dict["fc_conca2.weight"] = nn.init.kaiming_uniform_(model.fc_conca2.weight).float()
    fan_in, _ = nn.init._calculate_fan_in_and_fan_out(model.fc_conca2.weight)
    bound = 1 / math.sqrt(fan_in)
    state_dict["fc_conca2.bias"] = nn.init.uniform_(model.fc_conca2.bias, -bound, bound).float()
    state_dict["bn_conca2.weight"] = model.bn_conca2.weight.data.fill_(1).float()
    state_dict["bn_conca2.bias"] = model.bn_conca2.bias.data.fill_(0).float()
    state_dict["bn_conca2.running_var"] = model.bn_conca2.running_var.data.fill_(1).float()
    state_dict["bn_conca2.running_mean"] = model.bn_conca2.running_mean.data.fill_(0).float()
    model.load_state_dict(state_dict)
    if savemodel:
        torch.save(model.state_dict(), './pretrained/CorrCtrlNet_dir.pth')
    return model
setup_cornet2()
