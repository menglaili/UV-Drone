...
from utils.cumulative_err import gt_rel_translation, qua2rad, addrpose_interface
os.environ["CUDA_VISIBLE_DEVICES"] = '0'




def scene1_data(dataset_dir = "./data"):
    ref_dir = dataset_dir + "/" + "test_scene1_ref"
    ref_img_dir = os.path.join(ref_dir, "image")
    cur_dir = dataset_dir + "/" + "test_scene1_current/0"
    cur_img_dir = os.path.join(cur_dir, "image")
    ref_pose = np.loadtxt(os.path.join(ref_dir, "pose.txt"))
    cur_pose = np.loadtxt(os.path.join(cur_dir, "pose_current.txt"))
    with open("./data/test_action.txt", "w") as f:
        for i in range(cur_pose.shape[0]):
            ref_img_id = os.path.join(ref_img_dir, str(i) + ".png")
            cur_img_id = os.path.join(cur_img_dir, str(i) + "current.png")
            ref_pose_id = ref_pose[i, :]
            cur_pose_id = cur_pose[i, :]
            data_pair = (cur_img_id, ref_img_id, cur_pose_id, ref_pose_id)
            addrpose_interface(data_pair, f=f)
...
def gt_action_data(data_ref_dir = "./data/Ref_Data_free", mode = "train"):
    # Train and Val: img_addr1, img_addr2, action_x, axtion_y, delta_z, delta_theta
    # Test: img_addr1, img_addr2, action_x, axtion_y, delta_z, delta_theta
...
class DroneCRDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.action:
            ...
            if self.mode == "test":
                gt_pose = np.array(self.data_addr[idx][2:9], dtype = np.float)
