...
if __name__ == "__main__":
    ...
    with olympe.Drone("192.168.42.1") as drone:
        ...
        while not control.quit():
            ...
            time.sleep(0.02)
    # np.savetxt('./ctrl_seq.txt',np.array(ctrl_seq), fmt='%d', newline='\n')
...
# keycode 是数字，字母或标点等有包含信息的键盘符
# key 是无实际信息的操作符，如enter，cmd等其他
'''
def func1(key):
    if isinstance(key, KeyCode):
        print("on: keycode", key.char)
        return False
    elif isinstance(key, Key):
        print("on: key", key)
...
def func2(key):
    if isinstance(key, KeyCode):
        print("off: keycode", key.char)
    elif isinstance(key, Key):
        print("off: key", key)

with Listener(on_press = func1, on_release = func2) as l:
    l.join()
    print(l)
