...
def parse_arguments():
    ...
    parser.add_argument('--indir', type=str, default='/content/drive/My Drive/colab/mynet3_alpha0_sm/pclouds', help='input folder (point clouds)') #
    parser.add_argument('--outdir', type=str, default='/content/drive/My Drive/colab/mynet3_alpha0_sm/models/my_model_sm', help='output folder (trained models)') #
    parser.add_argument('--logdir', type=str, default='/content/drive/My Drive/colab/mynet3_alpha0_sm/logs', help='training log folder') #/content/drive/My Drive/colab/mynet3_alpha0_sm
    parser.add_argument('--trainset', type=str, default='trainingset_no_noise.txt', help='training set file name') #trainingset_no_noise.txt
    parser.add_argument('--testset', type=str, default='validationset_no_noise.txt', help='test set file name') #validationset_no_noise.txt
    parser.add_argument('--saveinterval', type=int, default='10', help='save model each n epochs')
    parser.add_argument('--refine', type=str, default='./my_single_scale_normal_model_300.pth', help='refine model at this path')
    ...
    parser.add_argument('--outputs', type=str, nargs='+', default=['oriented_normals'], help='outputs of the network, a list with elements of:\n'
    ...
    parser.add_argument('--alpha', type=float, default=0.5, help='ratio between two type loss')
...
def train_pcpnet(opt):
    ...
    if opt.refine != '':
        ...
        epoch_start = int(opt.refine[31:-4])
    ...
    optimizer = optim.Adam(pcpnet.parameters(), lr=opt.lr)
    # scheduler = lr_scheduler.MultiStepLR(optimizer, milestones=[50, 150, 300], gamma=0.1) # milestones in number of optimizer iterations
    pcpnet.to(device)
    ...
    for epoch in range(epoch_start, opt.nepoch):
        ...
        for train_batchind, data in train_enum:
            ...
            points = data[0]
            target = data[1]
            gtnor = data[-1].to(device)
            gtloss = data[-2].to(device)
            ...
            loss = compute_loss(
                pred=pred, gtnor = gtnor, gtloss = gtloss, alpha = opt.alpha,
                target=target, outputs=opt.outputs,
...
def compute_loss(pred, gtnor, gtloss, alpha, target, outputs, output_pred_ind, output_target_ind, output_loss_weight, patch_rot, normal_loss):
        ...
        if o == 'unoriented_normals' or o == 'oriented_normals':
            ...
            if o == 'unoriented_normals':
                ...
                if normal_loss == 'ms_euclidean':
                    ...
                    loss += ((o_pred-o_target).pow(2).sum(1).mean() * alpha + ((gtnor.sub(o_pred[:, None, :])).pow(2).sum(2).mean(1) - gtloss).abs().mean() * (1-alpha)) * output_loss_weight[oi]
                elif normal_loss == 'ms_oneminuscos':
