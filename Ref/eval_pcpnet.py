...
def parse_arguments():
    ...
    parser.add_argument('--indir', type=str, default='./PCCtest', help='input folder (point clouds)')
    ...
    parser.add_argument('--gpu_idx', type=int, default=-10, help='set < 0 to use CPU')
...
def eval_pcpnet(opt):
    ...
    for model_name in opt.models:
        ...
        regressor.load_state_dict(torch.load(model_filename, map_location='cpu'))
        regressor.to(device)
        regressor.eval()
