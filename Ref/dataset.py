...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.include_normals:
            gt_normal = torch.zeros(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 3, dtype=torch.float)
            gt_loss_list = torch.zeros(len(self.patch_radius_absolute[shape_ind]), 1, dtype=torch.float)
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            ...
            # output ground truth normal
            if self.include_normals:
                gt_normal[start:end, :] = torch.from_numpy(shape.normals[patch_point_inds, :])
                gt_loss_list[s] = (torch.from_numpy(shape.normals[patch_point_inds, :] - shape.normals[center_point_ind, :])).pow(2).sum(1).mean()

        # difference between center and group of the ground truth normal
        if self.include_normals:
            gt_loss = gt_loss_list.mean()
        ...
        for pfeat in self.patch_features:
            if pfeat == 'normal':
                ...
                gt_loss_feats = gt_loss_feats + (gt_loss,)
                gt_normal_feats = gt_normal_feats + (gt_normal,)
        ...
        return (patch_pts,) + patch_feats + (trans,) + gt_loss_feats + gt_normal_feats
